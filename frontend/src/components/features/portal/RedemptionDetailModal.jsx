    ...
    <Modal isOpen={isOpen} onClose={onClose} icon={Gift} title={redemption.reward?.name} maxWidth="max-w-md">
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <span className="font-mono font-bold text-ink">{redemption.points_cost?.toLocaleString()} pts</span>
          <StatusBadge size="lg" status={status} />
        </div>

        {redemption.student_note && (
          <div className="bg-porcelain border border-line rounded-xl p-3">
            <p className="text-[11px] font-mono uppercase tracking-wider text-muted font-semibold mb-1">
              Your note
            </p>
            <p className="text-sm text-muted">{redemption.student_note}</p>
          </div>
        )}

        {status === "PENDING" && (
          <p className="text-sm text-muted font-light">Your redemption is waiting for admin approval.</p>
        )}
        ...
        {status === "SCHEDULED" && fulfillment && (
          <div className="bg-gold/12 border border-gold/25 rounded-xl p-4 space-y-1">
            ...
            {fulfillment.meeting_url && (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="mt-2 inline-flex items-center gap-1.5 px-4 py-2 bg-pine hover:bg-moss text-paper text-xs font-semibold font-mono uppercase tracking-wider rounded-lg transition"
              >
        ...
        {status === "CANCELLED" && (
          <div className="flex items-start gap-2 text-sm text-muted bg-porcelain border border-line rounded-xl p-4">
