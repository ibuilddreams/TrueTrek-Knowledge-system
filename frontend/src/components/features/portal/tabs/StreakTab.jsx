...
function BackToPortalLink({ onBack }) {
...
export default function StreakTab({ onBack }) {
  ...
  if (isLoading) {
    return (
      <div className="space-y-4">
        <BackToPortalLink onBack={onBack} />
        <div className="rounded-2xl p-6 min-h-[40vh] flex items-center justify-center border bg-paper border-line">
  ...
  if (isError || !data) {
    return (
      <div className="space-y-4">
        <BackToPortalLink onBack={onBack} />
        <div className="rounded-2xl p-6 border bg-paper border-line">
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="rounded-2xl border p-5 sm:p-6 bg-paper border-line">
        <div className="flex items-center justify-between mb-5">
          <p className="flex items-center gap-1.5 text-xs font-mono uppercase tracking-wider text-muted">
          ...
          <div className="flex items-center gap-1.5 text-[10px] font-mono uppercase tracking-wider">
            ...
            <span className="w-3 h-3 rounded-[3px] border bg-porcelain border-line" />
            <span className="w-3 h-3 rounded-[3px] bg-gold border border-gold" />
            <span className="text-muted">Completed</span>
          </div>
        </div>

        <YearStreakCalendar days={data.days} />
      </div>
    </div>
  );
}
