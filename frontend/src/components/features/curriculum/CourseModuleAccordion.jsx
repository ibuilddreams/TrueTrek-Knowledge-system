...
function LessonRow({ lesson }) {
  ...
  return (
    <div className="flex items-center gap-3 rounded-xl px-2 py-2">
      <span className="flex items-center justify-center w-8 h-8 rounded-lg border shrink-0 bg-stone-50 text-stone-500 border-stone-200">
        <meta.icon className="w-4 h-4" />
      </span>
      <span className="min-w-0 flex-1">
        <span className="block text-[12.5px] font-medium truncate text-stone-700">
          {lesson.title}
        </span>
...
function ModuleRow({ module, isExpanded, onToggle }) {
  const lessons = module.lessons || [];
  const showProgress = typeof module.completion_percentage === "number";

  return (
    <div className="rounded-2xl border overflow-hidden border-stone-200 bg-stone-50/60">
      <button
        type="button"
        onClick={onToggle}
        aria-expanded={isExpanded}
        className="w-full text-left p-4 space-y-2 transition-colors hover:bg-stone-100/70"
      >
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            ...
            <h5 className="font-serif font-bold mt-0.5 truncate text-stone-900">
              {module.title}
            </h5>
          </div>
      ...
      <AnimatePresence initial={false}>
        {isExpanded && lessons.length > 0 && (
          ...
          >
            <div className="px-2 pb-3 pt-1 space-y-0.5 border-t border-stone-100">
              {lessons.map((lesson) => (
