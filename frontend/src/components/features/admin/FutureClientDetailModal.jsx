      ...
      {isLoading ? (
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3 text-xs">
            ...
            <div>
              <p className="text-[10px] font-mono uppercase text-stone-400 tracking-wider mb-1">Submitted</p>
              <p className="text-stone-700 font-semibold">{formatDate(application.submitted_at)}</p>
            </div>
            {application.reviewed_by && (
              <>
                <div>
                  <p className="text-[10px] font-mono uppercase text-stone-400 tracking-wider mb-1">Reviewed By</p>
                  <p className="text-stone-700 font-semibold">{application.reviewed_by.full_name}</p>
                </div>
                <div>
                  <p className="text-[10px] font-mono uppercase text-stone-400 tracking-wider mb-1">Reviewed At</p>
          ...
          {application.status === "REJECTED" && application.rejection_reason && (
            <div className="p-3 bg-rose-50 border border-rose-200 rounded-xl text-xs text-rose-700 leading-relaxed">
              <p className="text-[10px] font-mono uppercase tracking-wider mb-1 font-semibold">
                Rejection Reason
              </p>
          ...
          <div>
            ...
            <div className="space-y-2">
              {application.courses.map((course) => (
                ...
                >
                  <span className="text-xs font-semibold text-stone-800">{course.title}</span>
                  <span className="text-xs font-mono text-stone-500">${course.amount}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <p className="text-xs text-stone-400 font-light">Application unavailable.</p>
      )}
    </Modal>
  );
}
