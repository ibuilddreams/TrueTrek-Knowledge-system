  ...
  if (status === "failed") {
    return (
      <div className="bg-paper border border-line rounded-card shadow-elevated p-8 text-center max-w-lg mx-auto">
        ...
        <h2 className="text-xl font-serif font-bold text-ink mb-2">Failed to Load Statistics</h2>
        <p className="text-sm text-muted font-light mb-6">{error}</p>
        <button
          type="button"
          onClick={() => loadOverview({ force: true })}
          className="inline-flex items-center gap-2 px-5 py-3 bg-pine hover:bg-moss text-paper font-bold font-mono text-sm uppercase tracking-wider rounded-xl shadow-soft transition"
        >
