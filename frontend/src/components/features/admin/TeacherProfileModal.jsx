      ...
      {isLoading ? (
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between gap-3">
            ...
            <StatusBadge status={teacher.account_status} />
          </div>
          <div className="grid grid-cols-2 gap-3 text-xs">
            ...
            <div>
              <p className="text-[10px] font-mono uppercase text-stone-400 tracking-wider mb-1">Assigned Courses</p>
              <p className="text-stone-700 font-semibold">{courseCount}</p>
            </div>
          </div>
        </div>
      ) : (
        <p className="text-xs text-stone-400 font-light">Teacher profile unavailable.</p>
      )}
    </Modal>
  );
}
