    ...
    <Modal isOpen={isOpen} onClose={onClose} icon={ClipboardList} title="Enrollment Details" maxWidth="max-w-md">
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3 text-sm">
          <div>
            <p className="text-[11px] font-mono uppercase text-stone-400 tracking-wider mb-1">Student Email</p>
            <p className="text-stone-700 font-semibold break-all">{enrollment.student?.email}</p>
          </div>
          <div>
            <p className="text-[11px] font-mono uppercase text-stone-400 tracking-wider mb-1">Course</p>
            <p className="text-stone-700 font-semibold">{enrollment.course?.title}</p>
          </div>
          <div>
            <p className="text-[11px] font-mono uppercase text-stone-400 tracking-wider mb-1">Course Status</p>
            <StatusBadge size="lg" status={enrollment.course?.status} />
          </div>
          <div>
            <p className="text-[11px] font-mono uppercase text-stone-400 tracking-wider mb-1">Teacher</p>
            <p className="text-stone-700 font-semibold">{enrollment.teacher?.name || "Unassigned"}</p>
          </div>
          <div>
            <p className="text-[11px] font-mono uppercase text-stone-400 tracking-wider mb-1">Enrolled At</p>
