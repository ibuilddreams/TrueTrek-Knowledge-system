...
import { 
  Target, ArrowRight, Award, Flame, ShieldCheck, Zap, 
  MessageSquare, Send, X, RefreshCw, Sparkles, HelpCircle,
  ChevronDown
} from 'lucide-react';
...
export default function Home() {
  ...
  const [selectedConsultAdvisorId, setSelectedConsultAdvisorId] = useState('legal');
  ...
  const [selectedChatAdvisorId, setSelectedChatAdvisorId] = useState('recruiter');
  ...
  useEffect(() => {
    const advisor = ADVISOR_PERSONAS.find(a => a.id === selectedChatAdvisorId) || ADVISOR_PERSONAS[0];
    setChatMessages([
      {
...
How can I help you map out your high-compliance curriculum track or resolve specific training/academic bottlenecks today?`
  ...
  const handleSpeakToConsultAdvisor = async () => {
    ...
    const advisor = ADVISOR_PERSONAS.find(a => a.id === selectedConsultAdvisorId) || ADVISOR_PERSONAS[0];
...
Guide them, explain how the curriculum tiers relate to their query, and propose concrete physical/mental/legal protocols.`;
    ...
    try {
      ...
      const responseText = data.advice || `I apologize, but I am currently offline. Please try to send this message through our standard Secure student channels.`;
 ...
 = {
    athlete: {
      ...
      targetTiers: 'Tier 1 (Youth Athletics), Tier 2 (Recruiting Window), Tier 6 (Elite Pros & NIL)',
      objective: 'Secure high-impact athletic scholarship offers and structure durable NIL personal brand capitalization.',
    ...
    scholar: {
      ...
      targetTiers: 'Tier 3 (Early Academic Spike), Tier 4 (Common App Mastery), Tier 8 (Cognitive Bias)',
      objective: 'Master competitive academic portfolios, write elite college personal statements, and structure custom spikes.',
    ...
    founder: {
      ...
      targetTiers: 'Tier 5 (Pathfinders), Tier 7 (Startup Foundations), Tier 8 (Executive Mastery)',
      objective: 'Translate raw craft into scalable technological projects, secure seed financing, and build robust capital models.',
    ...
    parent: {
      ...
      targetTiers: 'Tier 1b (Parent Playbook), Tier 9 (Legacy Office Foundations)',
      objective: 'Navigate the high-pressure scouting maze safely while maintaining values-based family stewardship structures.',
  ...
  return (
    <div id="home-container" className="relative overflow-hidden w-full bg-[#1c1917] text-white">
      {/* Decorative ambient background */}
      <div id="ambient-sphere-1" className="absolute top-10 left-10 w-96 h-96 rounded-full bg-amber-600/15 blur-[120px] animate-pulse"></div>
      <div id="ambient-sphere-2" className="absolute bottom-10 right-10 w-96 h-96 rounded-full bg-stone-500/15 blur-[120px] animate-pulse"></div>

      {/* Hero Header Section */}
      <div id="hero-header-section" className="max-w-6xl mx-auto px-6 pt-24 pb-20 text-center relative z-10 flex flex-col items-center justify-center min-h-[85vh]">
        <motion.div 
          id="badge-banner"
        ...
        >
          Build Your <span className="italic text-stone-100 font-normal">Legacy</span>.
        </motion.h1>
        ...
        >
          The premier 11-tier educational roadmap and strategic incubator helping high-potential athletes, world-class scholars, and entrepreneurial pathfinders master real-world capital, law, and legacy.
        </motion.p>
        ...
        <div id="stats-dashboard" className="grid grid-cols-1 md:grid-cols-3 gap-8 w-full max-w-4xl mt-24 border-t border-stone-800/80 pt-16">
          <div id="stat-students" className="text-center md:text-left">
            <p className="text-4xl md:text-5xl font-[family-name:var(--font-display)] font-bold text-amber-500 tracking-tight">12,400+</p>
            <p className="text-stone-400 text-xs font-mono uppercase mt-2 tracking-wider">Active Global Pathfinders</p>
          </div>
          <div id="stat-admissions" className="text-center md:text-left border-y md:border-y-0 md:border-x border-stone-800/80 py-6 md:py-0 md:px-8">
            <p className="text-4xl md:text-5xl font-[family-name:var(--font-display)] font-bold text-white tracking-tight">98.4%</p>
            <p className="text-stone-400 text-xs font-mono uppercase mt-2 tracking-wider">Collegiate Selection Rate</p>
          </div>
          <div id="stat-capital" className="text-center md:text-right">
            <p className="text-4xl md:text-5xl font-[family-name:var(--font-display)] font-bold text-stone-200 tracking-tight">$145M+</p>
            <p className="text-stone-400 text-xs font-mono uppercase mt-2 tracking-wider">Aggregate Venture Valuation</p>
          </div>
        </div>
      </div>
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            <div id="audit-selector-group" className="lg:col-span-4 space-y-3">
              {[
                { id: 'athlete', label: 'Pro & Collegiate Athlete', accent: 'D1 Prospects' },
                { id: 'scholar', label: 'Scholarly Academic Elite', accent: 'Ivy League Candidates' },
                { id: 'founder', label: 'Venture Founder / Craftsman', accent: 'Scale Builders' },
                { id: 'parent', label: 'Parent Advisor / Steward', accent: 'Family Wealth Stewards' },
              ].map((item) => (
                <button
                ...
                >
                  <span className="font-medium text-sm text-stone-200">{item.label}</span>
                  <span className={`text-xs font-mono uppercase tracking-wider ${
                    selectedAuditProfile === item.id ? 'text-amber-500' : 'text-stone-500'
                  }`}>{item.accent}</span>
                </button>
              ))}
            </div>
            ...
            <div id="audit-results-container" className="lg:col-span-8 bg-stone-900 border border-stone-800/80 rounded-2xl p-6 md:p-8 relative min-h-[380px] flex flex-col justify-between">
              <div>
                <div className="flex flex-wrap items-center justify-between gap-4 border-b border-stone-800 pb-4 mb-6">
                  <div>
                    <span className="text-amber-500 text-xs font-mono tracking-widest uppercase block mb-1">RECOMMENDED SUB-TRACK</span>
                ...
                <div className="mb-6">
                  <p className="text-stone-300 text-sm leading-relaxed mb-6">
                    <span className="font-semibold text-stone-200">Objective Profile Target:</span> {auditRecommendations[selectedAuditProfile].objective}
                  </p>
                  <div className="bg-stone-950 p-3 rounded-lg border border-stone-850 mb-6 flex gap-2.5 items-center">
                    ...
                    <p className="text-xs text-stone-300">
                      <span className="font-mono font-medium text-amber-500 block uppercase tracking-wider text-[10px]">Primary Core Curriculum Licensing</span>
                      {auditRecommendations[selectedAuditProfile].targetTiers}
                    </p>
                  </div>

                  <p className="text-xs font-mono uppercase text-stone-400 tracking-wider mb-3">Incubator Strategic Checklist</p>
                  <ul className="space-y-2.5">
                    {auditRecommendations[selectedAuditProfile].roadmap.map((step, index) => (
                      <li key={index} className="flex items-start gap-2.5 text-stone-300 text-xs leading-relaxed">
              ...
              <div className="border-t border-stone-800 text-right pt-6 mt-8 flex flex-col sm:flex-row items-center justify-between gap-4">
                <span className="text-stone-400 text-xs font-light text-left">
                  Licensed across 40+ educational administrations. Verified athletic and academic compliance.
                </span>
                <button
      ...
      <section id="live-advisors-consultation-section" className="bg-[#141211] border-t border-stone-800/60 py-20 px-6 relative overflow-hidden z-10">
        ...
        <div className="max-w-6xl mx-auto space-y-12 relative z-10">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            <div className="lg:col-span-5 space-y-4">
              <label className="text-[10px] font-mono text-stone-500 uppercase tracking-widest block text-left">Choose Direct Session Advisor</label>
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0">
                        <p className="text-xs font-black text-stone-100 truncate">{advisor.name}</p>
                        <p className="text-[9px] font-mono text-stone-500 mt-0.5 uppercase tracking-wider truncate">{advisor.title.split('&')[0]}</p>
                      </div>
                    </div>
            ...
            <div className="lg:col-span-7 bg-stone-900/40 border border-stone-800/80 rounded-2xl p-6 md:p-8 space-y-6">
              ...
              <div className="space-y-2 text-left">
                <div className="flex justify-between items-center bg-[#141211]/30 p-1 rounded-md">
                  <span className="text-[10px] font-mono text-stone-300 uppercase tracking-wider block font-bold">Configure Your Custom Scenario</span>
                  <span className="text-[10px] font-mono text-amber-500 uppercase font-semibold">Active Session</span>
                </div>
                ...
                <div className="relative">
                  ...
                  />
                  <div className="absolute bottom-3 right-3 text-[9px] font-mono text-stone-500 select-none">{consultQuery.length} chars</div>
                </div>
              </div>

              {/* Suggestions pills */}
              <div className="space-y-2 text-left">
                <span className="text-[9px] font-mono text-stone-500 uppercase tracking-wide block font-semibold">Suggested Inquiries:</span>
              ...
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 pt-2 border-t border-stone-800/80">
                ...
                >
                  {isConsulting ? 'Analyzing Parameters...' : 'Ask Live Advisor'}
                  ...
                  >
                    ...
                    <div className="flex justify-between items-center border-b border-stone-850 pb-3 mb-4">
                      <p className="text-[10px] font-mono uppercase tracking-wide text-stone-400 flex items-center gap-2">
                        ...
                        Formal analysis by {ADVISOR_PERSONAS.find(a => a.id === selectedConsultAdvisorId)?.name}
                      </p>
                    ...
                    {isConsulting ? (
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="border-t border-stone-850 pt-4 mt-3 flex flex-col sm:flex-row items-center justify-between gap-3 text-stone-450 text-[11px]">
                          <span className="font-mono text-stone-500">Need immediate execution? Visit our syllabus tier files.</span>
                          <button
                            onClick={onExploreTiers}
      ...
      <section id="philosophies-section" className="py-20 px-6 max-w-6xl mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          <div className="p-6 border border-stone-800 bg-[#1c1917] rounded-xl text-left">
            <span className="w-10 h-10 rounded-lg bg-amber-600/10 text-amber-500 flex items-center justify-center font-[family-name:var(--font-display)] font-black text-lg mb-4">I</span>
            <h4 className="text-lg font-serif font-semibold text-white mb-2">Architectural Integrity</h4>
            <p className="text-stone-400 text-xs leading-relaxed">We provide real tactical metrics, redline contract examples, and legal codes. No placeholders or shallow advice blocks.</p>
          </div>
          <div className="p-6 border border-stone-800 bg-[#1c1917] rounded-xl text-left">
            <span className="w-10 h-10 rounded-lg bg-amber-600/10 text-amber-500 flex items-center justify-center font-[family-name:var(--font-display)] font-black text-lg mb-4">II</span>
            <h4 className="text-lg font-serif font-semibold text-white mb-2">Cognitive Science First</h4>
            <p className="text-stone-400 text-xs leading-relaxed">Mental capacity, diagnostic evaluation scores, and nervous system recovery habits drive genuine career longevity.</p>
          </div>
          <div className="p-6 border border-stone-800 bg-[#1c1917] rounded-xl text-left">
            <span className="w-10 h-10 rounded-lg bg-amber-600/10 text-amber-500 flex items-center justify-center font-[family-name:var(--font-display)] font-black text-lg mb-4">III</span>
            <h4 className="text-lg font-serif font-semibold text-white mb-2">Multi-Generation Focus</h4>
            <p className="text-stone-400 text-xs leading-relaxed">NIL deals and admission spikes are merely entry gates. We prepare you to build robust, generational wealth offices.</p>
          </div>
        </div>
      </section>

      {/* FAQ Section */}
      <section id="faq-section" className="py-24 px-6 border-t border-stone-800/60 bg-[#141211]/30 relative z-10">
        <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
          
          {/* Left Column: Heading and Badge */}
          <div className="lg:col-span-5 space-y-4 lg:sticky lg:top-24">
            <span className="text-amber-500 text-xs font-mono uppercase tracking-widest block">Institutional FAQ</span>
            ...
            <p className="text-stone-400 text-sm font-light leading-relaxed">
              Have specific inquiries regarding TrueTrek Learning&apos;s curriculum modules, security compliance pathways, and school licensing structures? Review our comprehensive advisory dossier.
            </p>
            <div className="pt-6 border-t border-stone-800/80 flex items-center gap-3">
              ...
              <div className="text-left">
                <p className="text-xs font-mono text-stone-300 font-bold uppercase tracking-wider">Direct Consultation Portal</p>
                <p className="text-[11px] text-stone-500 font-light mt-0.5">Need customized criteria evaluations? Utilize our live advisors or our virtual concierge chat desk below.</p>
              </div>
            </div>
          </div>
            ...
            {INDEX_FAQ_ITEMS.map((item, idx) => {
              ...
              return (
                ...
                >
                  ...
                  >
                    <span className="font-serif text-sm md:text-base font-semibold text-stone-200">
                      {item.question}
                    </span>
                    <span className={`w-8 h-8 rounded-full bg-stone-950 border border-stone-800 flex items-center justify-center shrink-0 text-stone-400 transition-transform duration-300 ${isOpen ? 'rotate-180 text-amber-500 border-amber-500/20 bg-amber-600/5' : ''}`}>
      ...
      <div id="floating-concierge-suite" className="fixed bottom-6 right-6 z-50 font-sans tracking-tight">
        <AnimatePresence>
          {isChatOpen && (
            <motion.div
            ...
            >
              ...
              <div className="bg-[#141211] border-b border-stone-850 px-4 py-3 flex items-center justify-between shrink-0">
                <div className="flex items-center gap-2.5">
                  ...
                  <div className="text-left min-w-0">
                    ...
                    <p className="text-[9px] font-mono text-stone-500 uppercase tracking-wide truncate">Live dispatch connection</p>
                  </div>
                </div>
              ...
              <div className="bg-stone-900 border-b border-stone-850 p-2 flex gap-1.5 overflow-x-auto shrink-0 scrollbar-none items-center justify-start">
                {ADVISOR_PERSONAS.map(adv => (
                  ...
                  >
                    <img src={adv.avatar} alt={adv.name} className="w-5 h-5 rounded-full object-cover shrink-0" referrerPolicy="no-referrer" />
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-stone-950/30 select-text">
                {chatMessages.map((msg) => (
                  ...
                  >
                    {msg.sender === 'advisor' && (
                      <img
                        src={ADVISOR_PERSONAS.find(a => a.id === selectedChatAdvisorId)?.avatar}
                        alt="Advisor"
                ...
                {isChatLoading && (
                  <div className="flex gap-2.5 justify-start">
                    ...
                    <div className="bg-stone-900 text-stone-400 text-xs py-2 px-3.5 rounded-xl border border-stone-850 flex items-center gap-2">
                      ...
                      <p className="font-mono text-[10px] animate-pulse">FORMULATING RECOMMENDATIONS...</p>
                    </div>
                  </div>
                )}
        ...
        <button
          ...
          className={`w-14 h-14 bg-gradient-to-tr from-amber-600 to-amber-800 text-white rounded-full flex items-center justify-center cursor-pointer shadow-2xl shadow-amber-950/50 hover:scale-105 active:scale-95 border-2 border-stone-900 transition-transform duration-200 select-none relative group ${
            isChatOpen ? 'bg-stone-900 from-stone-900 to-stone-950 border-stone-800' : ''
          }`}
        >
          {isChatOpen ? (
